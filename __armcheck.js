...
(async () => {
  ...
  for (const cfg of [{ t: 0, tag: 'bind' }, { t: 0.6, tag: 'anim' }]) {
    for (const view of ['front', 'side']) {
      await page.evaluate(({ t, view }) => {
        const THREE = window.THREE;
        const rec = buildEnemySkinned();
        const mesh = rec.mesh;
        if (t > 0) rec.mixer.update(t);
      ...
      await el.screenshot({ path: path.join(OUT, `arm_${cfg.tag}_${view}.png`) });
      console.log(`rendered ${cfg.tag}/${view}`);
    }
  }
