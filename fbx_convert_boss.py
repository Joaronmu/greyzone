...
FBX = "C:/Users/零零/Downloads/小boss.fbx"
...
# adaptive params -- tuned so output decimation level matches y-target (~37k clusters / ~3.5MB b64)
# boss native h=1.10 is larger than y-target, so cells are coarsened 1.42x to match cluster density
R_FINE=0.012
CELL_FINE=0.0066
CELL_COARSE=0.0198
print("CELL_FINE",CELL_FINE,"CELL_COARSE",CELL_COARSE)
isFine = verts[:,1] > (miny + 0.30*h)
print("fine verts",isFine.sum(),"coarse verts",(~isFine).sum())
...
# normalize: feet at 0, height 45, center xz (and center x at origin for clean placement)
...
open("boss_model.bin","wb").write(blob)
b64=base64.b64encode(blob).decode('ascii')
open("boss_model.b64","w").write(b64)
